...
def search_videos(query_string, orientation_landscape=True):
    print(f"apiiiii keyyyyyyy      njjcd beub{PEXELS_API_KEY}")
    url = "https://api.pexels.com/videos/search"
    headers = {
        "Authorization": 'fBVl69B5cMljqK5HuuaVu5QA3Gw4rsUkrumvJT2o1jLKYZEhObfjciBQ'
    }
    params = {
        "query": query_string,
    ...
    try:
            print(f"Sending request to Pexels API with query: {query_string}")
            response = requests.get(url, headers=headers, params=params)
            response.raise_for_status()  # Raise an exception for bad status codes
            json_data = response.json()
            print(f"Received response from Pexels API: {json.dumps(json_data, indent=2)[:500]}...")  # Print first 500 characters
            return json_data
