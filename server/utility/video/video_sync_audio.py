...
def create_tiktok_sync_audio_video(timestamps, conversation, audio_file_path, background_video_path, output_path, video_size=(1080, 1920), optional_cta_audio=None, cta_text=""):
    ...
    if magick_path:
        ...
        default_path = 'C:\Program Files\ImageMagick-7.1.1-Q16-HDRI\magick.exe'
        print(f"ImageMagick not found in PATH. Using default path: {default_path}")
        # change_settings({"IMAGEMAGICK_BINARY": r'C:\\Program Files\\ImageMagick-7.1.1-Q16-HDRI\\magick.exe'})
        os.environ['IMAGEMAGICK_BINARY'] = default_path

    if not os.path.exists(os.environ['IMAGEMAGICK_BINARY']):
        raise FileNotFoundError(f"ImageMagick binary not found at {os.environ['IMAGEMAGICK_BINARY']}. Please install ImageMagick or update the path.")
    ...
    for (t1, t2), text in timestamps:
        text_clip = TextClip(txt=text, fontsize=100, color="green", stroke_width=4, stroke_color="black", method="label")
    ...
    if cta_text:
        ...
        cta_clip =TextClip(txt=cta_text, fontsize=100, color="green", stroke_width=4, stroke_color="black", method="label")
        cta_clip = cta_clip.set_start(cta_start_time).set_duration(total_duration - cta_start_time).set_position(["center", 900])
